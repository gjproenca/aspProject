...
public partial class backend_admin_contact : System.Web.UI.Page
{
    ...
    protected void submitResponse_Click(object sender, EventArgs e)
    {
        if (textBoxId.Text != "" && textBoxSubject.Text != "" && textBoxResponse.Text != "")
        {
            try
            {
                SqlCommand cmd = new SqlCommand("UPDATE [Contact] SET [Active] = @active WHERE [IDContact] = @idcontact", conn);
                ...
                //FIXME: not showing message upon message
            ...
            Response.Redirect("~/backend/admin-contact.aspx");
        }
        else
        {
